...
const GetAdminByQuery = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/admin/getquery?a_id=${adminId}&a_name=${adminName}`);
      setAdmin(response.data);
    } catch (err) {
      setError("Failed to fetch admin details. Please check the ID and name.");
      setAdmin(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <h1 className="text-3xl font-bold mb-6 text-black">Get Admin by ID and Name</h1>
...
export default GetAdminByQuery;
